function visualize27DMeas(verts, landmarksIdx, insectPts)
    minY = min(verts(:,2));
    maxY = max(verts(:,2));
    minX = min(verts(:,1));
    maxX = max(verts(:,1));
    ...
    plotInsectPoints(h, verts(landmarksIdx.idx27_left,  :), 'yx', 10)
    plotStraightLine(h, verts(landmarksIdx.idx27_left,  :), 'y-', 2)
    plotInsectPoints(h, verts(landmarksIdx.idx27_right, :), 'yx', 10)
    plotStraightLine(h, verts(landmarksIdx.idx27_right, :), 'y-', 2)
